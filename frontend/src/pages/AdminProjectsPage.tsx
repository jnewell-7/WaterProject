...
const AdminProjectsPage = () => {
  ...
  const [editingProject, setEditingProject] = useState<Project | null>(null); // For edit functionality
  ...
  const handleDelete = async (projectId: number) => {
    ...
    if (!confirmDelete) return; // Exit if user cancels the delete action

    try {
      await deleteProject(projectId); // Call the API to delete the project
      setProjects(projects.filter((p) => p.projectId !== projectId)); // Remove the deleted project from the state
    } catch (error) {
            ...
            <tr key={p.projectId}>
              ...
              <td>
                <button
                  ...
                  onClick={() => setEditingProject(p)} // Set the project to edit
                >
                  Edit
                </button>
                <button
                  className="btn btn-danger btn-sm w-100 mb-1"
                  onClick={() => handleDelete(p.projectId)} // Handle delete
                >
                  Delete
                </button>
              </td>
